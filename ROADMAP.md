...
- [ ] **Run the test suite.** `SemanticCoherenceTests`, `WordFrequencyTests` and the `TestSupport` changes have never been compiled — correctness so far is hand-traced, which is not the same as green. `Package.swift` already declares the target, so `swift test` settles it; the generated Xcode project has no test target, which is the underlying gap (see Technical debt below).
...
- **No test target in the generated Xcode project.** `tools/generate_xcodeproj.py` compiles GameCore's sources directly into the app target, so there is no `GameCore` module for `@testable import` to attach to and the XCTest suite can only run via SwiftPM. Restructuring GameCore into a real framework target would let tests run from Xcode and CI, and is the fix worth doing before the suite grows further.
- **Balance simulator is a hand-maintained mirror** of the Swift engine (`tools/balance_sim.py`). It has already caught real bugs, but every rule now lives in two places and they can silently drift — the coherence bonus and word weighting each had to be ported by hand. Worth considering whether the simulator should drive the real engine instead.
